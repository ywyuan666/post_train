...
- 手动实现 DPO Loss 公式
- 加载微调后的 LoRA 模型作为策略模型 (policy)
- 加载基座模型作为参考模型 (reference)
- 计算真实偏好对的 log probability
- 交换验证
"""
import torch
...
def dpo_loss(
    ...
    """
    DPO 损失函数（手动实现）
    公式: loss = -log σ( β * ( (π(y_w) - π(y_l)) - (π_ref(y_w) - π_ref(y_l)) ) )
    """
...
def compute_response_log_prob(model, tokenizer, prompt: str, response: str, device) -> float:
    ...
    # 定位 response 部分的 token 范围
    prompt_tokens = tokenizer(prompt, add_special_tokens=False)["input_ids"]
    prompt_len = len(prompt_tokens)
    
    with torch.no_grad():
        outputs = model(**inputs)
        # logits 对应每个位置的预测（左移一位）
        logits = outputs.logits[0, prompt_len-1:-1, :]  # shape: (response_len, vocab)
...
def main():
    ...
    teacher_preset = "standard"                     # 使用 standard 策略微调的模型
    ...
    # ========== 1. 加载策略模型 (policy = 微调后的 LoRA) ==========
    print("👨‍🏫 加载策略模型 (微调后 LoRA)...")
    ...
    # ✅ 新增：显式将整个模型移动到目标设备（确保 LoRA 参数也到 GPU）
    policy_model = policy_model.to(device)
    policy_model.eval()
    print("✅ 策略模型加载完成")
    
    # ========== 2. 加载参考模型 (reference = 基座模型) ==========
    print("📚 加载参考模型 (基座)...")
    ...
    # ✅ 新增：同样显式移动到设备（虽然 device_map="auto" 已经处理，但为保险起见）
    ...
    print(f"Ref Chosen logp:      {rc:.4f}")
    print(f"Ref Rejected logp:    {rr:.4f}")
    ...
    # ========== 7. 额外验证：检查 chosen 是否真的更受偏好 ==========
    policy_diff = pc - pr
    ref_diff = rc - rr
    print(f"\n📊 Policy chosen - rejected: {policy_diff:.4f}")
    print(f"📊 Ref chosen - rejected:    {ref_diff:.4f}")
    if policy_diff > 0:
